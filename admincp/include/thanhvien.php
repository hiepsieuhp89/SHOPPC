...
if (isset($HTTP_GET_VARS['active'])&&(isset($HTTP_GET_VARS['n'])))
{
	...
	if ($keyactive=="administrator"||$keyactive=="admin")
	   ...
	   {	
	   	if($number==1)	$result_hieuluc = mysqli_query("UPDATE thanhvien SET hieuluc=0 WHERE user='$keyactive'");
		if($number==0)	$result_hieuluc = mysqli_query("UPDATE thanhvien SET hieuluc=1 WHERE user='$keyactive'"); 		
	   }
...
<?php
        $kq=mysqli_query("select count(*) from thanhvien"); 
