<?php
	$query=mysqli_query("select sanpham.*,giohang.* from giohang,sanpham where giohang.id=sanpham.id");
	while($r_q=mysqli_fetch_array($query))
	{
		$user_q=$r_q["user"];
  ...
  <tr height="30" bgcolor="#FFCC99">    
	  <td align="center" width="200" style="border-right:1px solid #333"><strong>Ngày đặt</strong></td>     
        <td align="center" width="200" style="border-right:1px solid #333"><strong>Khách hàng</strong></td>        
         ...
         <td align="center" width="70" style="border-right:1px solid #333"><strong>Giá</strong></td>
  </tr>  
  <?php
		$kq2=mysqli_query("select count(*) from giohang where tinhtrang='damua'"); 
...
//******************************** Nội Dung ********************************//
		...
		$conn = include "connect.php"; $kq3=mysqli_query($conn, $sql3);
		if(!$kq3)
			echo "";
		else{
...
?>
     <tr height="30">        
        ...
        <td align="center" width="70" style="border-right:1px solid #333; border-bottom:1px solid #333;"><a href="?m=dh&b=gh-chitiet&u=<?php echo "$user3"; ?>&ngay=<?php echo "$ngaydat"; ?>">Xem</a></td> 
      </tr>  
<?php }
	}
  ?>  
