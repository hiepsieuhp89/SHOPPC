...
//	echo "$name - $new<hr>";
	...
	{	
		if($id2!=$id){
			...
			$q=mysqli_query("update giohang SET id='$id2' where id='$id'");
			$q2=mysqli_query("update hoadon SET id='$id2' where id='$id'");
			//echo "$sql5<hr>";			
			if(!$kq5){
...
<?php
	function Getloaisp($idl)
	{
		...
		$kq2 = mysqli_query($sql2);
		$s2="";
		$n2=mysqli_num_rows($kq2);
		if($n2>0){
...
<?php
	...
	{
		$r=mysqli_fetch_array($kq);$id_loai=$r["id_loai"];$id_nhom=$r["id_nhom"];
