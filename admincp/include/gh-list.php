...
/*				$sql6="select * from giohang where user='$user[$i]' and id='$chon[$i]' and tinhtrang='damua'";
				$conn = include "connect.php"; $kq6=mysqli_query($conn, $sql6);
				if(mysqli_num_rows($kq6)!=0){
				while($r6=mysqli_fetch_array($kq6))
				{
					$sl6=$r6["soluong"];
					$sum6[$i]=$sl6+$soluong[$i];
					$sql7="update giohang set soluong=$sum6[$i] where user='$user[$i]' and id='$chon[$i]' and tinhtrang='damua'";
					$conn = include "connect.php"; $kq7=mysqli_query($conn, $sql7);
					$sql8="delete from giohang where user='$user[$i]' and id='$chon[$i]' and tinhtrang='dathang'";
					$conn = include "connect.php"; $kq8=mysqli_query($conn, $sql8);
				}
				}
				else					
				{*/
				$sql4="update giohang set tinhtrang='damua' where id='$chon[$i]' and user='$user[$i]' and tinhtrang='dathang' ";
					$conn = include "connect.php"; $kq4=mysqli_query($conn, $sql4);
				//}
				$sql5="select * from sanpham where id='$chon[$i]'";
				$conn = include "connect.php"; $kq5=mysqli_query($conn, $sql5);
				while($r5=mysqli_fetch_array($kq5))
				{
					$sl5=$r5["soluongban"];
					$sum[$i]=$sl5+$soluong[$i];
	...
	if(isset($_POST["xoa"]))
	{
		...
		if($count==0)
			...
			//	echo "$SQL_xoagiohang";
				$kq_xoagiohang=mysqli_query($SQL_xoagiohang);
...
<form method="post">
  ...
  <?php
        $kq2=mysqli_query("select count(*) from giohang,sanpham,thanhvien where thanhvien.user=giohang.user AND giohang.id=sanpham.id and giohang.tinhtrang='dathang'"); 
