...
$result_thanhvien=mysqli_query("SELECT * FROM thanhvien where user='$idkey'");
$row=mysqli_fetch_array($result_thanhvien);
$db_user=$row["user"]; 
?>
<?php
if(isset($_POST["act"])){	
	...
	$result_thanhvien2=mysqli_query($sql);
	$row2=mysqli_fetch_array($result_thanhvien2);
	...
	else
	{
		...
		{
			$pw=md5($pw);	
			$result_changepw = mysqli_query("UPDATE thanhvien SET pass='$pww' WHERE user='$user'");
