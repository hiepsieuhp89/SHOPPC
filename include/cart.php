...
if(isset($_POST["xoagiohang"]))
	{
		...
		if($xoacount==0)
			...
			{	
				...
				$kq_xoagiohang=mysqli_query(include 'connect.php',$SQL_xoagiohang);
				$n+=mysqli_affected_rows(include 'connect.php');
			}				
...
<?php
	...
	{
		$kq=mysqli_query(include 'connect.php',$sql);
...
<form method="post" name="form">
  ...
  <?php
	...
	$kq3=mysqli_query(include 'connect.php',$sql3);
	$i3=0;
	$tien3=0;
...
<table width="560" border="0" cellspacing="0" cellpadding="0" style="border:1px solid #333">
  ...
  <?php	
	$kq=mysqli_query(include 'connect.php',"select count(*) from giohang,sanpham where giohang.id=sanpham.id AND giohang.user='$user' AND giohang.tinhtrang='damua'"); 
...
//******************************** Nội Dung *****************************************//		
	...
	$kq2=mysqli_query(include 'connect.php',$sql2);
	$i=0;
	$tien=0;
	if(!$kq2)
		echo "";
